...
- Codex built-in image generation consumes Codex host quota, not an external image API charge. Missing image receipts mean unknown output, not unknown external billing; reconcile the original tool call and candidate files, then recover within the authorized attempt scope without requesting duplicate-fee approval solely for host quota. Preserve any actual external fallback's billing record separately; never relabel it as a Codex call or silently switch providers.
