...
Let the user enter provider keys in **Providers & API (供应商与 API)**, not conversation text or tracked configuration. Ark/Seedance and optional Doubao Speech remain defaults. Call `drama_list_providers` for current profiles, credentials status and input limits; see [provider configuration](docs/provider-settings.md). No key is needed for the isolated smoke test. Cloud models consuming local references additionally need a reachable asset URL: configure the existing asset bridge with Cloudflared (`AI_DRAMA_CLOUDFLARED_PATH` can select an installed executable) or a supported existing bridge. Inspect the [bridge implementation and supported configuration](plugins/ai-drama-studio/src/asset-bridge.mjs) before setup. Explain that approved reference bytes become accessible to the model service; do not expose the whole workspace or workbench publicly.
...
Outside Codex, require the user's configured image API key (unless working only with accepted existing images). Default to Ark Seedream; other image profiles are selectable from the live catalog. Ark's key may serve both Seedream images and Seedance video if the account has both entitlements. Remind the user to configure it in the workbench, never in chat.
