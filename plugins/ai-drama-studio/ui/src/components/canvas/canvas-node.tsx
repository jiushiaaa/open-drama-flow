...
import { ChevronRight, Copy, Download, Group, Image as ImageIcon, Music2, Play, Puzzle, RefreshCw, Star, Trash2, Video } from "lucide-react";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    return (
        ...
        >
            ...
            {showConnectionHandles && !referenceSelectionState && !isGroup ? <ConnectionHandleDot side="left" visible={hovered || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "target")} /> : null}
            {showConnectionHandles && !referenceSelectionState && (definition?.hasSourceHandle ?? true) && data.type !== CanvasNodeType.Config ? <ConnectionHandleDot side="right" visible={hovered || isSelected || isConnecting} onMouseDown={(event) => onConnectStart(event, data.id, "source")} /> : null}
...
function NodeContent(props: NodeContentRendererProps) {
    ...
    // OpenDramaFlow does not mount the upstream third-party plugin runtime.
...
function VideoNodeContent({ node, theme }: NodeContentRendererProps) {
    ...
    useEffect(() => { setFailed(false); setPlaying(false); }, [node.metadata?.content]);
    ...
    if (failed) return <div className="p-4 text-center text-sm">视频无法播放，请通过素材名称打开原文件检查。</div>;
    if (!playing) return <button type="button" className="relative flex h-full w-full flex-col items-center justify-center gap-3 rounded-[18px] bg-stone-900 text-white" aria-label={`播放视频：${node.title}`} onMouseDown={event => event.stopPropagation()} onPointerDown={event => event.stopPropagation()} onDoubleClick={event => event.stopPropagation()} onClick={() => setPlaying(true)}>
        {node.metadata.posterUrl ? <img src={node.metadata.posterUrl} alt="镜头参考图" className="absolute inset-0 h-full w-full object-contain opacity-70" /> : <Video className="size-8 opacity-30" />}
        <span className="relative flex size-12 items-center justify-center rounded-full border border-white/30 bg-white/10"><Play className="size-5" /></span><span className="relative text-xs opacity-60">点击播放</span>
    </button>;
    return <video src={node.metadata.content} poster={node.metadata.posterUrl} controls autoPlay preload="metadata" playsInline onError={() => setFailed(true)} onDoubleClick={event => event.stopPropagation()} className="h-full w-full rounded-[18px] bg-black object-contain" data-canvas-video={node.id} data-canvas-no-zoom />;
}
