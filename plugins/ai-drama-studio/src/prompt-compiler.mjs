...
function videoRequestFor(contract, settings, prompt, inputBindings) {
  ...
  // Seedance 2.5 source-video operations inherit source geometry. Edits also inherit duration; extensions keep the planned new duration.
  const seedance25 = /seedance-2-5-/.test(settings.seedanceModel);
  const followsSourceRatio = ["video-edit", "video-extend"].includes(contract.videoInputMode) && seedance25;
  const followsSourceDuration = contract.videoInputMode === "video-edit" && seedance25;
  const followsFirstFrame = ["image-to-video", "first-last-frame"].includes(contract.videoInputMode) && seedance25;
  ...
  return {
    ...
    parameters: compactObject({
      ratio: followsSourceRatio || followsFirstFrame ? "adaptive" : text(contract.videoParameters.ratio || settings.ratio, 20),
      duration: followsSourceDuration ? -1 : contract.duration,
      watermark: settings.watermark === true,
