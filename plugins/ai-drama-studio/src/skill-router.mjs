...
function conflictAdjustment(name, normalizedRequest) {
  ...
  if (referenceRebuild && name === "video-deconstruct") adjust(75, "reference-rebuild");
  if (referenceRebuild && ["video-prompting", "film-reference-prompt-writer"].includes(name)) {
  ...
  if (uiFlow && !productNarrative && name === "ui-motion") adjust(55, "ui-state-over-promo");
  if (uiFlow && !productNarrative && name === "digital-product-promo-generator") adjust(-35, "ui-state-over-promo");
  const digitalProduct = has("网站", "软件", "app", "saas", "数字产品", "网页", "录屏");
  if (productNarrative && digitalProduct && name === "digital-product-promo-generator") {
  ...
  if (polishedCommerceAd && !creatorLed && !lipProduct && !officialBrand && name === "minimalist-product-ad-generator") {
    adjust(62, "physical-product-commerce-ad");
  }
  if (polishedCommerceAd && !officialBrand && name === "brand-ad") {
    adjust(-24, "commerce-ad-over-official-brand");
  }
  if (physicalProduct && commerceAd && officialBrand && name === "brand-ad") {
    adjust(58, "official-brand-over-commerce-ad");
  }
  if (physicalProduct && commerceAd && officialBrand && name === "minimalist-product-ad-generator") {
  ...
  if (sourceAdaptation && adaptationBible && name === "film-shot") {
    adjust(24, "source-adaptation-shot-craft");
  }

  const poster = has("海报", "poster") && has("动起来", "动态", "微动", "做成视频");
  if (poster && name === "poster-motion-generator") adjust(60, "poster-over-title-sequence");
  if (poster && ["cinematic-title-sequence", "suspense-title-sequence-generator"].includes(name)) {
    adjust(-40, "poster-over-title-sequence");
  }

  const transcript = has("口播稿", "逐字稿", "旁白稿", "讲稿", "配音稿") && has("配画", "资料画面", "补镜头", "broll", "b-roll");
  if (transcript && name === "transcript-broll-planner") adjust(65, "transcript-over-explainer");

  const lyricLed = has("歌词", "字幕") && has("mv", "音乐视频", "节拍", "卡点");
  if (lyricLed && name === "music-video-subtitle-generator") adjust(65, "lyrics-over-performance-mv");
  if (lyricLed && name === "cool-music-video") adjust(-35, "lyrics-over-performance-mv");
...
function scoreEntry(entry, request) {
  ...
  const directName = normalize(entry.name);
  let score = directName && normalizedRequest.includes(directName) ? 80 : 0;
  const matches = [];
  const signals = [];
  const matchedLabel = [entry.label, ...(entry.routingLabels || [])]
    .find(label => normalize(label) && normalizedRequest.includes(normalize(label)));
  if (matchedLabel) {
    score += 34;
    signals.push(`label:${matchedLabel}`);
  }
...
export async function routeSkills(request, maxResults = 3) {
  ...
  // Match whole IDs (also inside $plugin:skill), never a shorter slug substring.
  const requestedIds = new Set((String(request || "").toLowerCase().match(/[a-z0-9]+(?:-[a-z0-9]+)+/g) || []).map(canonicalSkillName));
  const exactIds = skills.filter(skill => requestedIds.has(skill.name));
  const normalizedRequest = normalize(request);
  const explicit = exactIds.length ? exactIds : skills.filter(skill =>
    [skill.label, ...(skill.routingLabels || [])].some(label => normalize(label) === normalizedRequest)
  );
  const candidates = explicit.length ? explicit : skills.filter(skill => skill.name !== "ai-drama-producer");
  const ranked = candidates
    .map(entry => ({ entry, ...(explicit.length
      ? { score: 100, matches: [], signals: [`explicit:${entry.name}`] }
      : scoreEntry(entry, request)) }))
