...
export async function getToolCatalog(state, credentials = {}, { probeDependencies = probeLocalTools } = {}) {
  ...
  const entries = [
    ...
    { id: "external-text-generation", tool: "drama_prepare_provider_job", category: "generation", adapterImplemented: true, readiness: "consult-drama_list_providers", inputs: ["text"], outputs: ["image/video-candidate"], limits: "fal Wan video and fal/Replicate Flux images only; no advanced references; one frozen call per job; built-in image priority remains." },
