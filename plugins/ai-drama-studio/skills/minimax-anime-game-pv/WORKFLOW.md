...
- **声音**：已上传音频但用户未说明用途时，询问“这段音频主要如何参与制作？”，提供 `参考音乐节奏 / 参考台词或人声 / 只分析节奏与情绪 / 不使用这段音频`。前两项把真实音频先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5，第三项只使用分析结果并生成模型原生声音，第四项回到无用户音频路径；用户已经明确卡点、口型或情绪用途时直接继承。没有音频且用户未说明时，询问 `视频模型原生声音（推荐） / 静音视频 / 预留后期配乐空间`；预留后期配乐空间表示不生成 BGM，但允许环境声和动作声维持节奏。
...
先调用 `核验当前工具注册表与供应商合同` 获取当前视频能力，优先选择可用的 旧 MiniMax H3 路径。选定供应商、模型和模式后，立即读取 manifest 指向的 vendor knowledge card，并建立 Render Contract draft：
...
- `prompt_budget` 按统一优先级解析：manifest 明示限制 → vendor knowledge card 明示限制 → `references/prompt-rules.md` 的供应商 fallback。旧 MiniMax H3 在前两者未暴露字符上限时使用 6500 字符工作预算，为已知 7000 字符硬上限预留执行余量；该值在首次编译前锁定，不是失败后的临时压缩规则。
...
2. 使用 `drama_update_plan（写入项目实体）` 返回的 `contentLength` 与 Render Contract 的 `prompt_budget` 做精确字符校验。
