...
根据当前 Seedance 能力与动作复杂度安排片段，15 秒只是节奏建议，不是统一模型上限。超过单次能力或动作过密时再拆分，保留接点并说明拼接方式。
...
automatic 下对照用户目标自检后继续；manual、用户要求阶段审核或路线存在实质歧义时等待确认。可调整环绕半径、穿越点、飞行高度、俯冲速度与画幅，但不得静默改变目标。

原始素材必须有使用授权，生成的参考图片必须先验收入库；这些边界不因 automatic 消失。
...
根据确认后的分镜和参考素材生成各片段。图片作为主体/场景锚点，可选视频作为运镜风格参考。默认视频模型为当前 Seedance 适配器，参数按实际能力校验。将角色图和场景图作为参考，并在有角色时围绕飞行角色追拍/跟飞路线写 Prompt。每个片段保持在所选模型支持的时长范围内，最长不超过 15 秒。

如果片段失败、主体丢失、结尾角色消失、角色朝向错误或依赖随意推拉镜头，改用更明确的角色持续可见、面朝飞行方向、路线驱动冲击、近距离穿越/侧倾/俯冲设计，或更简单的穿越约束后在原范围、剩余上限与实际失败状态明确后才考虑一次有界重试；仍失败则记录原因并停止该任务；不自动切换模型或增加调用上限。如果角色过于抢镜，降低角色展示权重，转为场景优先。如果窄缝穿越失败，用近距离擦过或揭示式环绕替代，不强行执行不可能路径。
