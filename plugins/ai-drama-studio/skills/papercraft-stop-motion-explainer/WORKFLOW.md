...
当本 Skill 从方案/提示词推进到实际视频生成时，默认按总控冻结请求并执行 Seedance 2.5 作为视频生成模型。其他模型仅在明确要求且真实接入时使用，不继承旧供应商参数。
...
默认在每个关键阶段自检后连续推进，不逐阶段弹卡片。只有 manual、用户要求阶段审核或确有改变结果的缺项时才询问；已给出的时长与方向直接继承。生成预览图留在库外，用户验收后才入库或参与后续生成。
