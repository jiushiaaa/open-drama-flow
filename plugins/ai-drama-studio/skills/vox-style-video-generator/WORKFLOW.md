...
文档结尾记录方案自检结论；automatic 下在范围内继续制作，manual 或用户指定审核时才等待方案确认。主视觉和关键帧候选仍在库外展示并经用户验收后入库；独立音乐可用SeedAudio 1.0语音任务mode=music，先核验配置/权限、冻结一次调用并试听。
...
- 使用有授权的纯器乐音轨，或在 provider-native 视频请求中表达配乐方向；独立音乐可用SeedAudio 1.0语音任务mode=music，冻结一次调用，试听后显式绑定，不编造音频文件。
