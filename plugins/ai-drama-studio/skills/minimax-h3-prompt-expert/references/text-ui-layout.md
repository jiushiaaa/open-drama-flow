...
```text
【推荐链路】
先生成 / 上传 1 张 UI、文字、构图参考图，再把它作为 @图片1 输入 旧 H3。
...
【旧 H3 视频提示词】
说明 @图片1 是 UI/文字/构图/色彩系统参考，旧 H3 必须保持其结构和文字稳定，然后写时间线、动效、声音、禁止项。

【一致性校验】
检查参考图提示词与 旧 H3 提示词中的文字、角色名、颜色身份、布局位置是否一致。
```
...
## Seedance 2.5 prompt（从旧 H3 结构迁移） Rule With Reference Image

```text
@图片1 是 UI/文字/构图/色彩系统参考图。这张内容必须按“图片”来理解，不能按文字重新处理。旧 H3 必须保持 @图片1 中的 UI 结构、文字内容、字体层级、按钮位置、色彩身份和画面构图稳定清晰。视频中可以让面板滑入、按钮高亮、光标点击、加载条推进、角色动作和背景展开，但所有清单文字必须与 @图片1 完全一致，不新增随机文字。
```
