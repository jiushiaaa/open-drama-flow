...
把用户粗略的视频创意、素材列表、关键帧需求或编辑指令，改写成生产可用的 Seedance 2.5 提示词（从旧 H3 结构迁移）。

默认用中文输出最终 prompt，除非用户明确要求其他语言，或项目本身需要英文 / 多语言文字。最终提示词只输出当前 Seedance 2.5 可执行格式。

## 不可妥协的目标

写出具体、可视化、分段清晰、可控的 Seedance 2.5 prompt（从旧 H3 结构迁移）。好的 Seedance 2.5 prompt（从旧 H3 结构迁移） 不是把用户一句话润色得更漂亮，而是一份压缩版导演简报：定义参考素材、主体身份、视觉系统、运动路径、声音和失败防控。
...
提问或写最终 Seedance 2.5 prompt（从旧 H3 结构迁移） 前，先选择并读取匹配的 reference 文件。不要把全部细节塞在主 skill 里。
...
先判断最接近的 旧 H3 任务类型：T2VA、I2VA、FL2VA、L2VA、全能多模态参考、源视频编辑、音频 / 对白 / 卡点、准确文字 / UI / 布局、产品 / Logo、MV / performance、游戏 / UI / HUD、动作 / 打戏、短剧 / 对白、字体包装 / MG，或其他具体模式。
...
用户回答后，把回答视为已确认约束。根据用户选择的路线生成最终提示词，并且只输出当前 Seedance 2.5 可执行格式。

必要澄清未完成前，不输出最终 prompt。如果不需要澄清，则直接输出官方格式 prompt。

## 只输出官方 旧 H3 格式

根据任务选择对应经核验的 Seedance 2.5 结构。不要使用旧版创作者友好中文分栏模板。
...
- 不要只靠风格名，要写可见的画面特征。
- 人脸一致性关键时，不要在没有人物参考图的情况下要求 旧 H3 强保脸。
