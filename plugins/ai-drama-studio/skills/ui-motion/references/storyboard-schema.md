...
This is a local craft/stitcher document, not a provider request schema. Actual generation uses the producer ShotSpec v2 and [Seedance guide](../../ai-drama-producer/references/seedance-prompting.md). Durations below are examples, not a mandatory 15-second split. Source files must really exist and be bound by asset ID/version before production; never import the example brands as user project data.
...
Add a `takes` array whose entries are numbered segments. Segment 1 uses the all-purpose reference image. Segment N+1 can only use the verified tail frame of segment N as its first frame. The `motion_prompt` strings below are abbreviated only to show the segment fields; before generation, expand every segment independently with `motion-prompt-writing.md` to the density required by its local duration. Never send these abbreviated strings to `Seedance 冻结请求与策略授权执行链（见总控执行规则）`.
...
```json
{
  ...
  "style_anchor_ref": "style_anchor.json",
  "music": {
    "source": "existing-authorized-audio",
    "mood": "warm acoustic",
    "prompt": "gentle acoustic morning, soft nylon guitar, 70 BPM, no vocals, 30 seconds",
    "file": "audio/bg.mp3"
  },
  "narrative": {
    "summary": "Example two-segment treatment. Preserve native sound or mix an existing authorized music asset according to the sound plan; no independent music model is connected."
  },
  "brand_assets": {
    "reference_image": {
...
- `resolution` — choose a currently supported video resolution from drama_get_capabilities; 720p is an example, not inferred from aspect ratio.
...
- `music` — optional existing authorized audio, not an instruction to generate independent music. Record the actual file and source binding only when it exists. Preserve needed native dialogue and sound effects; duration does not decide sound policy.
...
Mode coherence: choose one segment or a chain according to current Seedance limits and action complexity. Chains require real contiguous lineage. Optional music is an existing authorized source; it is not required by duration.
