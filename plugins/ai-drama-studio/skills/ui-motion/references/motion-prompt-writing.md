...
Read this file before writing any `motion_prompt` for `Seedance 冻结请求与策略授权执行链（见总控执行规则）`. Treat the prompt as a compact director's treatment with a visible timeline, not as a mood paragraph or a list of animation keywords.
...
Before calling `Seedance 冻结请求与策略授权执行链（见总控执行规则）`, confirm:
