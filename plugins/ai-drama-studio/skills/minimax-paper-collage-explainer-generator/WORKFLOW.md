...
- 默认视频生成模型：`旧 MiniMax H3`，除非用户明确指定其他模型、该模型不可用，或硬性能力要求排除它
...
优先先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5，因为它的原生音频能力适合本流程的同步拼贴音效。用户指定其他模型且满足已确认要求时遵循其选择；如果模型失败、不可用或缺少必要能力，说明差异并让用户选择等效方案。
