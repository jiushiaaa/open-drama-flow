...
- 声音：ASR 与标准音色 TTS 已接入，先查 `drama_get_capabilities`；没有语音 Key 时使用 Seedance 原生声音并实际听音检查。SeedAudio 1.0独立音乐通过语音任务mode=music单次冻结并试听后绑定；声音克隆、3D 编辑器与剪辑软件工程写入尚未接入，不伪造结果。
