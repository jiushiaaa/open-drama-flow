...
For a single segment, call `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）` with `mode: "multimodal"` and put the user's image in `reference_image_paths`. Send duration, aspect ratio, and resolution through the tool's duration/vendor metadata, and pass the complete `motion_prompt` unchanged. Do not put the image in `first_frame_image` and do not summarize, shorten, or collapse the timed beats at tool-call time.

For a continuation chain, generate segment 1 through the same `mode: "multimodal"` all-purpose-reference path. Then, strictly in sequence, call `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）` for segment N+1 with `mode: "i2v"` and `first_frame_image` equal to the verified tail frame of segment N. Each segment keeps its own `sequence`, `continuation_of`, prompt, input file, output clip, and extracted tail frame. Never call the segment-1 multimodal opening procedure again for segment N+1.
...
```bash
node .Codex-v2/skills/ui-motion/scripts/stitch.mjs \
...
Replace the `node .Codex-v2/.../stitch.mjs` call with `node ".Codex-v2\\skills\\ui-motion\\scripts\\stitch.mjs"`. The script is pure Node.js. Install ffmpeg via `winget install Gyan.FFmpeg` or `choco install ffmpeg`. Music generation is still limited to >15s by default and uses the Hub-native `用户授权音乐或已接入的音乐供应商；未接入时不得生成` tool; video generation uses `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）` with the same reference/continuation rules on Windows.
