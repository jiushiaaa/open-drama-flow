...
Read this file before writing any `motion_prompt` for `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）`. Treat the prompt as a compact director's treatment with a visible timeline, not as a mood paragraph or a list of animation keywords.
...
Before calling `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）`, confirm:
