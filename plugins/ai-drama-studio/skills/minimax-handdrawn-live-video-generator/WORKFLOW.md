...
本 Skill **先按用户输入语言整理 prompt，并把 旧 MiniMax H3 作为用户确认后的生成步骤**。用户明确确认 旧 H3 生成前，不生成视频。不要为单纯 prompt 写作转交 planner 或 executor。最终 prompt 的语言必须跟随用户输入的主语言：用户用中文就写中文，用户用英语就写英语，用户用日语就写日语；混合输入时使用占主导的语言；难以判断时使用当前对话语言。只有用户要求保留的专名、模型名或字面参数可以原样保留。
...
- 下一步建议必须使用同语种邀请用户先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5 生成 15 秒 16:9 视频。中文示例：`下一步建议：如果你确认这个 prompt，我可以继续用 旧 H3 模型生成 15 秒 16:9 视频。`
- 用户明确确认使用默认 旧 MiniMax H3 或明确指定的其他模型前，不生成视频、图片、音频、分镜或中间资产。
- 默认模型是 旧 MiniMax H3。如果用户明确指定其他模型，经过能力检查后遵循用户选择。除非用户要求，否则不要在创意 prompt 内添加模型参数。用户指定的模型生成失败时，最多针对性重试一次，仍失败则切换 旧 MiniMax H3 或其他可用模型，不要反复重试同一模型。
...
先按要求输出与用户输入主语言一致的最终 prompt。随后只添加一句同语种下一步建议，邀请用户确认后继续用 旧 MiniMax H3 生成 15 秒 16:9 视频。不要包含标题、清单、文件名或画布交付说明。用户后续确认 旧 H3 生成后，再生成视频并检查接触、连续变形、慢半拍追拍和非恐怖手绘质感是否成立。
