...
- 声音：ASR 与标准音色 TTS 已接入，先查 `drama_get_capabilities`；没有语音 Key 时使用 Seedance 原生声音并实际听音检查。SeedAudio 1.0 音乐/歌曲可用 mode=music 冻结一次请求，需语音 Key、实际服务权限、试听后显式绑定；声音克隆、3D 编辑器与剪辑软件工程写入尚未接入，不伪造结果。
...
交付一条统一、可观看的二次元漫画或游戏 PV，而不是只交付分析、分镜或提示词。主 Agent 负责素材判断、视觉收敛、分镜、Prompt 编译、执行合同和最终验收；实际视频生成只通过一个 OpenDramaFlow Video Agent `task` 执行。不创建 Stage Execution Plan，也不派 planner/executor 或下游 Agent 改写已确认的视频提示词。
...
| `Storyboard Approval` | 用户通过当前会话确认 对当前 Storyboard Package 给出的批准回执；必须绑定 `storyboard_revision`，存在九宫格时同时绑定 `preview_revision`。它只授权 Prompt 编译，不授权视频生成。 |
...
`visual_authority_status` 只允许 `not_applicable | pending | approved`；`storyboard_status` 与 `prompt_status` 只允许 `empty | pending | approved`。新生成视觉权威图、Storyboard Package 或 Prompt 首次创建或内容变化时为 `pending`；只有当前会话确认 对当前版本返回有效确认后才为 `approved`。后续产物不能反向替代前一状态的批准。
...
素材分析后记录画风来源。用户已明确给出方向或授权参考时直接继承，不重复发起 Style Authority Gate；automatic 下由 Agent 对照目标自检，只有实际来源冲突、缺失或用户要求人工决策时才使用以下选项。生成的视觉权威图始终需要用户验收入库。
...
- **声音**：已上传音频但用户未说明用途时，询问“这段音频主要如何参与制作？”，提供 `参考音乐节奏 / 参考台词或人声 / 只分析节奏与情绪 / 不使用这段音频`。前两项把真实音频按总控冻结请求并执行 Seedance 2.5，第三项只使用分析结果并生成模型原生声音，第四项回到无用户音频路径；用户已经明确卡点、口型或情绪用途时直接继承。没有音频且用户未说明时，询问 `视频模型原生声音（推荐） / 静音视频 / 预留后期配乐空间`；预留后期配乐空间表示不生成 BGM，但允许环境声和动作声维持节奏。
...
生成图片的人工验收始终保留。下列分镜和提示词两道人工门禁仅适用于 manual 或用户明确要求逐阶段审核；automatic 下对应为 Agent 自检与版本冻结，不写成用户批准回执。旧文中的 approved 字段不能伪造写回 Harness，实际授权由总控 requestDigest 和 automatic-policy 记录。

1. `style_authority_kind=generated_asset` 时，视觉权威图生成并检查后设置 `visual_authority_status=pending`，用当前会话确认 提供：使用当前视觉参考继续、调整画风表现、调整配色、调整参考图处理、调整角色一致性。确认后记录 `visual_authority_revision` 并设置为 approved；existing_asset 路径设置 `visual_authority_status=not_applicable / visual_authority_revision=not_applicable`。
...
| 静止系 MAD、歌词文字或动态字体是主体 | `seedance-visual-design` 或对应 Skill |
| 音乐、说唱、时尚表演或 MV 是主叙事 | `cool-music-video` |
| 通用动态设计 MG | `seedance-visual-design` 或对应 Skill |
...
1. 用 `本地媒体核验（view_image、ffprobe 或 FFmpeg）` 分析相关图片、视频和音频，并先为每份素材建立候选 Ref Capsule：`{ id, candidate_roles, contributes, take }`。`candidate_roles` 是 `subject | scene | style | layout | action | audio` 的集合；分析结果不能替代 Style Authority Gate。
...
先调用 `drama_get_capabilities` 获取当前视频能力，优先选择可用的 Seedance 2.5 路径。选定供应商、模型和模式后，立即读取 manifest 指向的 运行时能力合同 card，并建立 Render Contract draft：
...
audio / reference_slots / videoParameters / prompt_budget / final_output_count
```
...
Prompt 长度由当前运行时能力校验；不要继承未经验证的固定 7000 字符上限或为凑下限堆砌描述。
...
需要生成视觉权威图时，先查询 image 能力，再按 `references/visual-inputs.md` 的公共 Visual Authority Contract 与当前 `visual_artifact_type` 编译生图 Prompt，并用 `Codex Image Gen 任务领取、生成、目检与回填流程` 落实权威身份贡献、`style_source`、适用的配色字段、`reference_processing` 和画面形式。生图 Prompt 必须区分必须保留的身份、允许适配的表现和禁止继承的载体信息；不得使用“优化原图”“保持原构图”或不区分路径的“生成宣传海报/KV”等泛化表达。生成或覆盖资产时创建新的 `visual_authority_revision` 并设置 `visual_authority_status=pending`。

生成后先用 `本地媒体核验（view_image、ffprobe 或 FFmpeg）` 按当前 `visual_artifact_type` 检查。所有路径都要保持身份边界并移除不应继承的载体污染；指定方向和风格融合路径要命中所选风格；角色重设计要符合允许变化范围；内部默认视觉路径必须通过单主题色预算、强调色预算、纯白/近白肤色、硬边黑影、平面色块、低信息背景和动画截图感检查，同时不能只是原图换色，也不能生成精致宣传海报、三视图或最终分镜。未通过时不得提升为 `style_authority`，应在同一视觉阶段修正对应合同后重新生成。
...
1. 用 `drama_update_plan（写入项目实体）` 创建或覆盖同一个 `storyboard_review.md`，写入完整 Brief、素材贡献、`style_source → style_authority`、`visual_artifact_type`、配色权威、饱和度策略、参考图处理、声音方案、全部 Panel、Logical Shot 分组和无空档的 Master Timeline。
...
manual 或用户指定分镜审核时，本阶段等待可信确认；automatic 下完成分镜自检即可编译，并记录准确版本。任何模式都不得把普通回复或 Agent 自检当作用户批准的图片/记忆。
...
`final_video_prompt` 的文档标题、章节标题、Logical Shot 叙事名称和正文统一使用运行时注入的 `working_language`；模型 ID、模式名、素材标签、文件路径和用户精确文字保持原文。规划文档可以保留专业章节和 Logical Shot 分节，但提交模型的 videoPrompt 必须编译为当前 Seedance 自然语言，遵守实际长度与参数限制，不能照搬旧供应商 Markdown 协议。

按以下顺序完成可执行编译：

1. 编译候选 Prompt，并用 `drama_update_plan（写入项目实体）` 创建或覆盖同一个 `final_video_prompt.md` 文档节点；正文必须与准备派单的 Prompt 字符级一致。
2. 使用实际编译器返回的请求与错误检查长度，不能虚构 drama_update_plan 返回 contentLength 或传入旧文档节点 API 字段。
...
执行必须由总控接管：读取实际方案及已验收资产，编译 ShotSpec 请求，冻结 digest/上限，再调用 `drama_authorize_and_start_paid_batch`。automatic 按策略记录，manual 按可信确认执行。不要求 OpenDramaFlow Video Agent、task 或独立子 Agent；本 Skill 的 Panel/Storyboard 文档是制作资料，不能当成当前 MCP 的额外字段。以实际请求摘要保证执行内容未被暗改。
...
生成后用 `本地媒体核验（view_image、ffprobe 或 FFmpeg）` 检查人物身份、剪影、与已确认 `style_authority` 的一致性、`palette_authority` 与 `saturation_policy`、内部默认视觉路径的主题色/强调色预算和角色配色策略、动作可读性、文字、Shot 承接、转场、节奏、声音关系、最终停留和异常伪影。
