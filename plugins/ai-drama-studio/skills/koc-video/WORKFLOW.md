...
领域规则位于 `references/`。skill 加载后会返回 `Base directory for this skill: <dir>`；进入对应阶段前，用绝对路径 `读取本地文件或 drama_get_state <dir>/references/<name>.md`。读取完成后才能依赖其内容生成，不把读取与依赖它的生成放在同一批。
...
只确认会明显改变成片或生成成本的决定。确认在需要用户验收或必要澄清时，不在聊天里要求用户“回复确认”。默认有四个确认点：
...
读取 `source-routing.md`。对每个来源 ref 用一次 `本地媒体核验（view_image、ffprobe 或 FFmpeg） type:"both"`，识别产品、可见事实、技术质量、结构与尺度风险，以及它能承担的 reference 职责。
...
分镜表包含画面、逐字口播和用户能听见的声音，用 `drama_update_plan（写入项目实体）` 落画布，再用 `question` 确认。用户口述修改时同步更新节点；用户编辑节点后，生成前用 `drama_get_state（读取项目实体）` 重读最终版本。修改后重新确认。
