...
把用户确认的影片方案、准确文字、字体系统、镜头、转场、可见特效和完整 15 秒音乐编译成一个可直接按总控冻结请求并执行 Seedance 2.5 的中文 Prompt。默认设置：`Seedance｜15 秒｜16:9｜720p（提交前校验）｜同步音乐与环境声｜中英双语屏幕文字`。
...
`question` 被拒绝、取消、报错、无返回值或返回值无法解析时，保持当前 `awaiting_*` 状态并停止。不得猜默认值、不得继续写最终 Prompt、不得调用 `Seedance 冻结请求与策略授权执行链（见总控执行规则）`。
...
options:
- 单条 15 秒体验（推荐）｜一次 Seedance 2.5 生成，内部包含多个镜头、准确文字和完整配乐，直接交付一条成片。
- 15 秒以上 / 多片段完整预告｜退出本 Skill，返回主 Agent 规划多段生成、剪辑和组装。
```

- 选择 `15 秒以上 / 多片段完整预告`：设置 `TRAILER_FLOW_STATE=returned_to_main_agent`，明确告诉用户将由主 Agent 继续规划，然后退出本 Skill；不得继续问风格、语言、文字或按总控冻结请求并执行 Seedance 2.5。
...
总结必须把风格名转成 Seedance 2.5 可看见、可听见的具体特质，不能只重复“国风、复古、高级、艺术感”。现有 Reference 继续提供提示词结构、节奏、镜头、转场、字体层级和音乐完整性；`AD_HOC_STYLE_SUMMARY` 只负责当前项目的画面表面与排除项。用户改选主风格时按新模板重新总结。
...
只有 `reference_scope_confirmed=true` 后才能进入 `awaiting_proposal_confirmation`。此时不调用 `question`，也不生成最终 Seedance 2.5 Prompt。写确认稿之前必须先按第 5 节判定 `OUTPUT_MODE` 和主路由，并立即读取：
...
检测到附件后先用 `本地媒体核验（view_image、ffprobe 或 FFmpeg）` 只读分析内容、镜头、文字、色彩与声音。媒体分析请求必须只提取可验证事实：人物身份与服装、地点元素、色盘、光线、现有文字、构图层级和可见动作；不得让分析器决定参考范围、替整支成片设计慢推/侧移/同一场景延展，也不得把它给出的创意建议当成已确认事实。参考范围只由 Gate 3 决定。
...
1. [seedance-prompt-contract.md](references/seedance-prompt-contract.md)
...
1. 调用 `drama_get_capabilities` 确认 Seedance 2.5 的videoInputMode、时长、画幅、分辨率、声音开关和参考角色。
...
3. 按 [seedance-prompt-contract.md](references/seedance-prompt-contract.md) 写出中文 `final_seedance_prompt`。最终 Prompt 只保留 Seedance 2.5 能直接看见、听见和执行的自然语言。
4. 长度遵守当前运行时限制，不为凑固定下限增加重复描述。首次成稿后计数一次；越界时只允许一次确定性修整并再计数一次。不得第三次试算、来回重写或向用户直播计数。
5. 派单前必须显式得到以下内部结果：
...
prompt_length=validated_by_current_runtime
...
任一项不成立：保持或退回对应状态，说明缺哪一项；不得调用 `Seedance 冻结请求与策略授权执行链（见总控执行规则）`。这条检查是 `Seedance 冻结请求与策略授权执行链（见总控执行规则）` 的必要前置，不因用户催促、工具可接受短 Prompt 或上一次生成成功而豁免。
...
7. 完整展示锁定后的唯一 Prompt，并在同一 assistant turn 直接调用 `Seedance 冻结请求与策略授权执行链（见总控执行规则）`；不得通过 `task`、video agent、运行时能力合同 或临时文件摘要、翻译、压缩或二次改写。
...
- 音乐是否只锁风格、情绪、2–4 个音色锚和完整能量弧，让 Seedance 2.5 自主作曲，同时有开场动机、推进、高潮和明确终止？
- `final_seedance_prompt` 是否只计数最多两次、处于 当前运行时长度限制，并与实际调用为同一字符串？
