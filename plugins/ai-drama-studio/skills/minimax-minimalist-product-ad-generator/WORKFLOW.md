...
7. **视频模型策略**
   - 默认模型是 旧 MiniMax H3，因为它通常更容易生成干净的 Apple 感产品近景、留白和高级产品片镜头语言。
   - 启动问询时不要让用户选择视频模型；只需在已采用的启动结果中展示“默认模型：旧 MiniMax H3”。
   - 如果用户明确指定其他模型，经过能力检查后遵循用户选择。
   - 用户指定的模型生成失败时，最多针对性重试一次；然后说明能力差异，让用户选择 旧 MiniMax H3 或其他等效模型，不要静默切换。

启动问询是强制 Gate。即使用户已经上传了商品素材，也只能跳过“请上传素材”这一步；不能跳过其余启动问询。进入产品分析、锚定图、分镜或视频生成前，仍须一次性确认商品款式、目标时长、画幅比例、苹果风模板和画面文案方式。视频模型不是启动问询问题：优先先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5，并在已完成的启动结果中展示。如果用户说“你看着办 / 直接做”，可采用推荐默认值，但仍须先展示已采用的选择。
...
- 默认生成一条 旧 H3 全画幅产品片，而不是逐镜生成多个首帧视频，除非用户明确要求其他构造方式。
- 视频阶段默认开启 旧 MiniMax H3 原生音频，除非用户明确要求其他模型或无声。音乐提示词使用 Apple 风科技配乐方向，让画面节奏和声音一起生成。
...
优先先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5 原生音频，因为它能在一次生成中对齐产品动作和声音。如果音乐不好听、太吵、太弱、不同步，或用户要求重新配乐，就用当前可用的音乐模型生成较长的独立器乐并替换音轨。选择付费或严格控时方案前，先确认明显的成本和输出差异。
