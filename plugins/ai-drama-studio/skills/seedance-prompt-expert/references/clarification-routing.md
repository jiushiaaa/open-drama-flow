...
Use when the user's request is short, broad, ambiguous, or missing information that would materially change the Seedance 2.5 prompt skeleton.

## Intake Order

1. Identify the request type.
2. Evaluate whether the text input and provided references satisfy the need.
3. Ask only for material missing decisions; reuse explicit constraints and state safe defaults. Default automatic execution does not require a style menu or per-stage confirmation.
4. After the user chooses, write the final prompt in natural-language videoPrompt, with validated ShotSpec parameters and media bindings kept separately.
...
If the request is simple, low-risk, and already satisfies the need, skip questions and write the natural-language prompt directly.
...
Ask no more than three targeted questions at a time. Prefer multiple choice. Put the recommended option first. Ask only questions that change the prompt structure. If clarification is required, output only the questions and wait; do not include the final prompt yet.
...
After the user answers, treat answers as confirmed constraints and write the final prompt in natural-language videoPrompt, with validated ShotSpec parameters and media bindings kept separately.
