...
- [Generative Kinetic Typography Engine](https://github.com/zfryrgnci/kinetic-typography)：把字体轮廓拆成大量粒子/顶点，用 physics、noise 和 shader 让字形发生波动、破碎与聚合，并把交互位置作为响应源。转译为 Seedance 2.5：把“物体高光/边缘/动作轨迹”设为粒子来源，先聚合成完整实心白名单文字，再只做一次局部脉冲；不要要求 Seedance 2.5 生成连续数学模拟。
- [TypeFlow Kinetic Typography Maker](https://www.typeflow.studio/kinetic-typography-maker)：提供 flip-board 逐字落位、wave/color surge、CRT scanline 和 spring/scale reveal 等明确模板。转译为 Seedance 2.5：每个 Shot 只挑一种揭示机制，例如一条扫描线从左到右揭示整词，或粒子逐字落位；写清起点、方向、完成时间和复位，不能把 flip、wave、CRT、glitch 全部叠加。
- [卡点文字快闪案例](https://www.xiaohongshu.com/discovery/item/680f6bf70000000009014498)：蓝色色场中，中文实心字、英文超大透明字、圆环、球体和波点网格在多个状态中重组。转译为 Seedance 2.5：Display A 使用实心几何字，Display B 使用低对比轮廓/透明大字，Utility 保持稳定；球体、波点网格和圆环属于 GRAPHIC KIT，每一状态只让一个组件承担主运动。
