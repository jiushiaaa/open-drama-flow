...
## TTS2 performance and dialect direction

Optional `contextText` freezes one non-empty instruction (at most 500 characters) separately from the spoken `text`. For verified TTS2 Uranus voices the adapter sends `req_params.additions` as a JSON string containing `context_texts: [contextText]`; only one context is supported because the official API uses only the first entry. ASR/music and non-TTS2 voices reject this option. The instruction is included in the request digest and manual confirmation, never silently prepended to spoken dialogue.

Official reference: https://docs.volcengine.com/docs/DoubaoVoice/HTTPChunkedSSEUnidirectionalStreaming-V3?lang=zh and https://docs.volcengine.com/docs/DoubaoVoice/Tonelist-1?lang=zh . The catalog lists Cantonese for Yunzhou/Xiaotian 2.0; this is capability evidence, not proof of correct generated Cantonese. Do not invent `explicit_dialect=yue`: the documented explicit_dialect enum is restricted to other dialects/voices. Use supported context direction and require actual pronunciation review. The first project samples lacked this control and were rejected by the user; corrected requests still need listening acceptance.
