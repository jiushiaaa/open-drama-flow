...
把用户确认的影片方案、准确文字、字体系统、镜头、转场、可见特效和完整 15 秒音乐编译成一个可直接先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5 的中文 Prompt。默认设置：`旧 MiniMax H3｜15 秒｜16:9｜2K｜24fps｜同步音乐与环境声｜中英双语屏幕文字`。
...
`question` 被拒绝、取消、报错、无返回值或返回值无法解析时，保持当前 `awaiting_*` 状态并停止。不得猜默认值、不得继续写最终 Prompt、不得调用 `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）`。
...
options:
- 单条 15 秒体验（推荐）｜一次 旧 H3 生成，内部包含多个镜头、准确文字和完整配乐，直接交付一条成片。
- 15 秒以上 / 多片段完整预告｜退出本 Skill，返回主 Agent 规划多段生成、剪辑和组装。
```

- 选择 `15 秒以上 / 多片段完整预告`：设置 `TRAILER_FLOW_STATE=returned_to_main_agent`，明确告诉用户将由主 Agent 继续规划，然后退出本 Skill；不得继续问风格、语言、文字或先把旧 旧 H3 意图改写成连续自然语言，再通过审批链调用 Seedance 2.5。
...
总结必须把风格名转成 旧 H3 可看见、可听见的具体特质，不能只重复“国风、复古、高级、艺术感”。现有 Reference 继续提供提示词结构、节奏、镜头、转场、字体层级和音乐完整性；`AD_HOC_STYLE_SUMMARY` 只负责当前项目的画面表面与排除项。用户改选主风格时按新模板重新总结。
...
只有 `reference_scope_confirmed=true` 后才能进入 `awaiting_proposal_confirmation`。此时不调用 `question`，也不生成最终 旧 H3 Prompt。写确认稿之前必须先按第 5 节判定 `OUTPUT_MODE` 和主路由，并立即读取：
...
1. 调用 `核验当前工具注册表与供应商合同` 确认 旧 MiniMax H3 的合法 mode、时长、画幅、2K、音频和参考槽位。
...
3. 按 [h3-prompt-contract.md](references/h3-prompt-contract.md) 写出中文 `final_h3_prompt`。最终 Prompt 只保留 旧 H3 能直接看见、听见和执行的自然语言。
...
任一项不成立：保持或退回对应状态，说明缺哪一项；不得调用 `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）`。这条检查是 `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）` 的必要前置，不因用户催促、工具可接受短 Prompt 或上一次生成成功而豁免。
...
7. 完整展示锁定后的唯一 Prompt，并在同一 assistant turn 直接调用 `drama_request_paid_batch → 用户批准 → drama_resume_paid_batch（Seedance 2.5）`；不得通过 `task`、video agent、vendor card 或临时文件摘要、翻译、压缩或二次改写。
8. 调用传全：`vendor="MiniMax"`、合法 `mode`、`model_id="旧 MiniMax H3"`、`prompt=final_h3_prompt`、`filename`、`duration=15`、`vendor_params={ratio, resolution:"2K", generate_audio:true}` 以及合法素材字段。
...
- 音乐是否只锁风格、情绪、2–4 个音色锚和完整能量弧，让 旧 H3 自主作曲，同时有开场动机、推进、高潮和明确终止？
