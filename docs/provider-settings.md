...
工作台 **供应商与 API** 分为「默认生成模型」与「供应商与密钥配置」：前者选择新任务默认使用谁，后者管理各家的凭据与接口。保存密钥不会自动切换默认模型。选择供应商后填写密钥即可；腾讯云需要 SecretId 与 SecretKey。MiniMax 国内与国际站、不同供应商的凭据分别保存，不会自动复用或转发。
...
“用量详情”只展示调用、趋势、供应商／模型汇总与定价。供应商筛选后，模型选项随之收窄；同名模型按供应商隔离，CNY 与 USD 不相加。本地工具配置位于独立的“工具”页面；界面不再提供账户账单同步设置。
...
历史费用补算接口 `POST /api/usage/backfill` 保留给 Agent 在用户要求时调用，前端不再提供按钮。补算使用当前价格及已保存用量，保留原记录和依据，不重算已有价格、不产生模型调用；失败或未完成任务不补算，缺少用量的金额仍留空。实账记录与账单接口在后台保留，不与估算相加；用量页不展示实账或录入操作。此版本不自动抓取全平台账单或价格，不包含 Codex 订阅账单。已有火山只读账单接口仅作为后台兼容能力保留，使用独立账单凭据。

## 工具与新手指南

左侧入口依次为“用量详情 → 供应商与 API → 工具 → 新手指南”。“工具”集中管理 Real-ESRGAN、Video Depth Anything 与 FFmpeg：超分面板展示本地任务和恢复操作，不是消费记录；视频深度保存 Python、项目目录和权重路径，验证路径不等于安装依赖或运行推理，标题旁的信息图标可悬浮或键盘聚焦查看说明；FFmpeg 面板展示本机依赖与滤镜检测结果。

“新手指南”提供两级目录和章节跳转，覆盖初次配置、项目与素材、生成与验收、费用、本地工具和常见问题。说明页不会向项目写入演示内容。
