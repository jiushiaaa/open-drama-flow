...
> 历史迁移记录：下文版本、技能数量与测试统计仅属于 2026-09-03，不代表当前安装。最新整合见 [2026-09-12 制作迭代](production-iteration-20260912.md)，当前行为以总控合同为准。
...
ASR 与标准音色 TTS 已接入，依赖实际配置/权限；没有语音 Key 可用 Seedance 原生声音。SeedAudio 1.0独立音乐按语音任务mode=music冻结并试听后绑定。声音克隆、3D 编辑器与剪辑软件工程写入未接入。常规剪辑、字幕和混音使用 FFmpeg。
