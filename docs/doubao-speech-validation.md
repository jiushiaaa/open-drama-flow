...
Seedance 2.5 仍负责视频与主要原生声音。豆包语音 Key 可选，独立 DPAPI 加密保存；没有它不阻止 Seedance 生产。ASR 用于对白/字幕核对，TTS 用于官方预置音色的旁白与补录。SeedAudio 1.0音乐/歌曲已接入mode=music单次冻结请求，预置TTS音色可按请求选择；声音克隆仍未接入。
