...
先识别宿主：Codex 使用内置图片工具；其他 Agent（`AI_DRAMA_AGENT_HOST=generic`）需要配置图片 API Key，通过供应商任务工具准备、执行和下载候选，验收后才导入素材库。此路径目前支持方舟 Seedream、fal、Replicate 的文生图，不支持参考图编辑。下方 Codex 制作案例是原有实践，不是必须使用 Codex 的限制。
